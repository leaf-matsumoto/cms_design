<div class="row" style="border-top: 1px solid #d2d6de;">
    <div class="col-md-12">

        <div class="col-md-6" style="text-align:center;margin:100px 0;">
            <!-- <div class="col-sm-12">                  -->
                <div class="btn-title">コンセマスタ同期</div>
                <button type="submit" class="btn btn-lg btn-chain disabled" style="margin:20px 0;color:#fff;background-color:#696969!important;width:80%;">同期実行</button>
                <div class="chain-result" style="font-size:19px;">同期中...</div>
            <!-- </div> -->
        </div>

        <div class="col-md-6" style="text-align:center;margin:100px 0;">
            <!-- <div class="col-sm-12"> -->
                <div class="btn-title">クーポンマスタ同期</div>
                <button type="submit" class="btn btn-info btn-lg btn-chain" style="margin:20px 0;background-color:#696969!important;width:80%;">同期実行</button>
                <div class="chain-result" style="font-size:19px;color:red;">同期失敗：2022/07/07 11:50</div>
            <!-- </div> -->
        </div>

    </div>

</div>

